...
const ParallaxCarousel = ({ demostrationImages }) => {
  ...
  const goToPrevSlide = () => {
    goToSlide(
      (currentSlide - 1 + demostrationImages.length) % demostrationImages.length
    );
  };
    ...
    <div className="relative w-full h-96 overflow-hidden rounded-lg shadow-xl bg-gray-900">
      ...
      <div className="relative w-full h-full">
        {demostrationImages?.map((image, index) => (
          ...
          >
            <div
              className="absolute w-full h-full bg-cover bg-center transform transition-transform duration-700"
              style={{
